...
//				+ Math.pow(line.y2 - line.y1, 2))));
	...
	public void renderTooltip(Graphics g, Point mouseLocation) {
		...
		switch (this.effect) {
		case NOISE:
			g.drawString("Creates noise of " + this.effectValue + " radius",
		...
		if (this.areaOfEffect >= 512) {
			g.drawString("Very large area of effect", mouseLocation.x + 20,
					mouseLocation.y - 80);
		} else if (this.areaOfEffect >= 256) {
			g.drawString("Large area of effect", mouseLocation.x + 20,
					mouseLocation.y - 80);
		} else if (this.areaOfEffect >= 128) {
			g.drawString("Normal area of effect", mouseLocation.x + 20,
					mouseLocation.y - 80);
		} else if (this.areaOfEffect >= 64) {
			g.drawString("Small area of effect", mouseLocation.x + 20,
					mouseLocation.y - 80);
		} else {
			g.drawString("Very small area of effect", mouseLocation.x + 20,
					mouseLocation.y - 80);
		}

		if (this.range >= 640) {
			g.drawString("Very far throwing range", mouseLocation.x + 20,
					mouseLocation.y - 55);
		} else if (this.range >= 480) {
			g.drawString("Far throwing range", mouseLocation.x + 20,
					mouseLocation.y - 55);
		} else if (this.range >= 320) {
			g.drawString("Normal throwing range", mouseLocation.x + 20,
					mouseLocation.y - 55);
		} else if (this.range >= 160) {
			g.drawString("Close throwing range", mouseLocation.x + 20,
					mouseLocation.y - 55);
		} else {
			g.drawString("Very close throwing range", mouseLocation.x + 20,
					mouseLocation.y - 55);
		}
	}
}