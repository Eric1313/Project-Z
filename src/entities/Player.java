...
public class Player extends Mob {
	...
	public void useItem() {
		...
		// if (currentTick - this.getLastItemTick() > newItem.getRechargeTime())
		// {
		// this.setLastItemTick(currentTick);
		//
		// for (long frame = currentTick; frame < currentTick +
		// newItem.getSwingSpeed(); frame++) {
		// Line2D.Double line = new Line2D.Double(new Point(this.position.x +
		// 16, this.position.y + 16),
		// new Point((int) (this.position.x + 16 + newItem.getRadius() *
		// Math.cos(angle + (Math.PI / 3) * ((frame - currentTick -
		// newItem.getSwingSpeed() / 2) / (newItem.getSwingSpeed() / 2.0)))),
		// (int) (this.position.y + 16 - newItem.getRadius() * Math.sin(angle +
		// (Math.PI / 3) * ((frame - currentTick - newItem.getSwingSpeed() / 2)
		// / (newItem.getSwingSpeed() / 2.0))))));
		// // System.out.println(frame - currentTick - newItem.getSwingSpeed() /
		// 2);
		...
		// if (!newItem.isEmpty()) {
		// long currentTick = this.game.getTickCount();
		// if (currentTick - this.getLastItemTick() > newItem.getRateOfFire()) {
		// this.setLastItemTick(currentTick);
		//
		// int d = 32 * 16;
		//
		...
		// new Point((int) (this.position.x + 16 + d
		// * Math.cos(angle)),
		// (int) (this.position.y + 16 - d
		// * Math.sin(angle))));
		//
		...
		for (int x = chunkX - 3; x < Math.min(chunkX + 4,
			...
			for (int y = chunkY - 3; y < Math.min(chunkY + 4,
				...
				for (Iterator<Zombie> iterator = chunkMap[x][y].getZombies()
					...
					if (line.intersects(zombie.getPosition().x,
							zombie.getPosition().y, 32, 32)) {
						zombiesCollided.add(zombie);
					}
				}

				for (Iterator<Entity> iterator = chunkMap[x][y]
						.getSolidEntities().iterator(); iterator.hasNext();) {
					Entity entity = iterator.next();
					if (line.intersects(entity.getPosition().x,
							entity.getPosition().y, 32, 32)) {
